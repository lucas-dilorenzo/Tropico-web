...
import { eliminarCategoria } from "@/lib/actions/catalogo";

export default function EliminarCategoria({ catId, nombre }: { catId: string; nombre: string }) {
  ...
  async function handleEliminar() {
    setLoading(true);
    const result = await eliminarCategoria(catId);
    if (result.error) {
      alert(result.error);
      setLoading(false);
      setConfirming(false);
